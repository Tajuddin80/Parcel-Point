  ...
  const handleImageChange = (e) => {
    ...
    if (selectedFile) {
      ...
      if (selectedFile.size > 2 * 1024 * 1024) {
        Swal.fire("Too Large", "Max size 2MB", "info");
        return;
      }
  ...
  return (
    <div className="flex flex-col items-start">
      <label htmlFor="profileImage" className="cursor-pointer relative group">
        <img src={preview} alt="Upload profile"
      ...
      {preview !== defaultProfile && (
        <button onClick={handleRemoveImage} className="mt-2 btn text-xs text-red-500 hover:underline">Remove Image</button>
      )}
    </div>
  );
});
